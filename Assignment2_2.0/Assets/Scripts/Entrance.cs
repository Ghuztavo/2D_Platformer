...
public class Entrance : MonoBehaviour
{
    ...
    {
        BlocksStatus();
        if (GameManager1.instance.stage == 1 && type ==1 )
        {
            if (isMoving)
            {
    ...
    {
        if (type ==1 && GameManager1.instance.stage ==1)
        { 
            if(GameManager1.instance.gemCollected && GameManager1.instance.cherryCollected)
            {
                Debug.Log("Blocks Moving");
                isMoving = true;
            }
        }
    }
